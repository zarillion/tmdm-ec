...
local function IsMessageDisplayed()
	for index = 1, 3 do
		if MESSAGE_TIMERS[index] then
			return true
		end
	end
	return false
...
function ns.actions:BannerMessage(index, message, duration)
	...
	if frame then
		...
		TMDM_MessageFrame:Show()

		if MESSAGE_TIMERS[index] then
			MESSAGE_TIMERS[index]:Cancel()
		end

		MESSAGE_TIMERS[index] = C_Timer.NewTimer(duration, function()
			frame:SetText("")
			MESSAGE_TIMERS[index] = nil
			if not IsMessageDisplayed() then
				TMDM_MessageFrame:Hide()
			end
		end)
	end
end
